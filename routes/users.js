...
const {verifyToken, verifyTokenAndAuthentication}= require('./verifyToken.js')
...
router.post('/login',async (req,res)=>{
    try{
        ...
        },process.env.JWT_CODE,{expiresIn:"1d"})
...
router.put("/:id",verifyTokenAndAuthentication,async (req,res)=>{
   ...
   try {const updatedUser = await Tuser.findByIdAndUpdate(req.params.id,{$set:req.body},{new:true})
